	...
	for name, mover in pairs(movers) do
		if mover:IsShown() then
			...
			if ZenAlign.Position then
				ZenAlign.Position:Save(mover) -- Save mover's position as the frame's position
				-- Wait, Position:Save uses frame:GetName(). Mover name is ZenAlignMover_...
				-- We need to save for the TARGET frame using MOVER's coordinates.
			...
			if ZenAlign.Position then
				ZenAlign.Position:Restore(frame)
			end
